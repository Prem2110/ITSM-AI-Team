...
function FieldSection({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="mb-3">
      <div className="text-2xs font-semibold text-surface-400 uppercase tracking-wider mb-1">{label}</div>
      <div>{children}</div>
    </div>
  )
}
...
function EventItem({ event, userMap }: { event: IncidentEvent; userMap: Record<string, string> }) {
  ...
  if (event.event_type === 'state_change') {
    const from = meta.from_state ?? meta.old_state ?? '?'
    const to = meta.to_state ?? meta.new_state ?? '?'
    return (
      <div className="flex items-center gap-2 py-1 text-2xs text-surface-500">
        <span className="w-1.5 h-1.5 rounded-full bg-surface-300 flex-none" />
        <span>
          <span className="font-semibold text-surface-600">{actor}</span>
          {' '}changed state{' '}
          <span className="font-mono">{from.replace(/_/g, ' ')}</span>
          {' → '}
          <span className="font-mono font-semibold">{to.replace(/_/g, ' ')}</span>
  ...
  if (event.event_type === 'field_update') {
    ...
    return (
      <div className="flex items-start gap-2 py-1 text-2xs text-surface-500">
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      >
        ...
        {editTitle ? (
          <input
            ...
            className="flex-1 text-sm font-semibold bg-transparent border-b border-surface-400 focus:outline-none focus:border-surface-700 min-w-0"
          />
        ) : (
          <span
            className={`flex-1 text-sm font-semibold text-surface-900 truncate ${isAgent && !isClosed ? 'cursor-pointer hover:text-surface-600' : ''}`}
            onClick={() => {
              if (isAgent && !isClosed) {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto flex flex-col min-w-0">
          {/* Description */}
          <div className="px-5 pt-4 pb-4 border-b border-surface-100">
          ...
          {/* Comment box */}
          {!isClosed && (
            <div className="flex-none px-5 py-3 border-t border-surface-100 bg-surface-50">
              {isAgent && (
                <div
                  className="flex mb-2"
              ...
              <textarea
                ...
                className="w-full text-xs border border-surface-200 bg-white px-2 py-1.5 focus:outline-none focus:border-surface-400 resize-none"
                style={{ borderRadius: 2 }}
              />
        ...
        >
          ...
          <FieldSection label="Category">
            {isAgent && !isClosed ? (
              ...
              <span className="text-xs text-surface-700">{incident.category}</span>
            )}
          </FieldSection>
          ...
          <FieldSection label="Source">
            <span className="text-xs text-surface-700 capitalize">{incident.source.replace(/_/g, ' ')}</span>
          </FieldSection>

          <div className="border-t border-surface-200 my-3" />

          <FieldSection label="SLA Due">
            {incident.sla_resolution_due ? (
              <span className={`text-xs ${incident.sla_breached && !isResolved ? 'text-red-600 font-medium' : 'text-surface-700'}`}>
                {new Date(incident.sla_resolution_due).toLocaleString('en-US', {
                  month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit',
                })}
                {incident.sla_breached && !isResolved && (
                  <span className="ml-1 text-2xs font-semibold text-red-500">(breached)</span>
                )}
              </span>
            ) : (
          ...
          <FieldSection label="Created">
            <span className="text-xs text-surface-700" title={incident.created_at}>
              {relativeTime(incident.created_at)}
            </span>
          </FieldSection>

          <FieldSection label="Updated">
            <span className="text-xs text-surface-700" title={incident.updated_at}>
          ...
          {incident.resolved_at && (
            <FieldSection label="Resolved">
              <span className="text-xs text-surface-700" title={incident.resolved_at}>
          ...
          {incident.closed_at && (
            <FieldSection label="Closed">
              <span className="text-xs text-surface-700" title={incident.closed_at}>
            ...
            <>
              ...
              {incident.resolution_notes && (
                <FieldSection label="Resolution Notes">
                  <p className="text-xs text-surface-700 whitespace-pre-wrap leading-relaxed">{incident.resolution_notes}</p>
                </FieldSection>
              )}
            </>
          )}
